...
def get_harmonic_mean(num):
    ...
    return (len(num) - zeros_found)/ average if average != 0 else 0.00
...
def accuracy_full_game(positions, analysis_depth):
    print(f"ANALYZING EACH MOVE TO DEPTH {analysis_depth} ...")
    _engine = "engines/ChessEngine.exe"
    ...
    for i in range(len(win_rate_lists_mid) - 1):
        if (i % 2) == 0:
            accuracy_lists_mid.append(max(accuracy(win_rate_lists_mid[i], win_rate_lists_mid[i + 1]),0.00))
        elif (i % 2) != 0:
            accuracy_lists_black_mid.append(max(accuracy(win_rate_lists_black_mid[i], win_rate_lists_black_mid[i + 1]),0.00))
    for i in range(len(win_rate_lists_end) - 1):
        if (i % 2) == 0:
            accuracy_lists_end.append(max(accuracy(win_rate_lists_end[i], win_rate_lists_end[i + 1]),0.00))
        elif (i % 2) != 0:
            accuracy_lists_black_end.append(max(accuracy(win_rate_lists_black_end[i], win_rate_lists_black_end[i + 1]),0.00))
    for i in range(len(win_rate_lists) - 1):
        if (i % 2) == 0:
            accuracy_lists.append(max(accuracy(win_rate_lists[i], win_rate_lists[i + 1]),0.00))
        elif (i % 2) != 0:
            accuracy_lists_black.append(max(accuracy(win_rate_lists_black[i], win_rate_lists_black[i + 1]),0.00))

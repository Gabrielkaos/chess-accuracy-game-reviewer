...
class Game:
    ...
    def draw_info(self):
        ...
        self.win.blit(text, (WIDTH, 60))

        #if in check
        in_check = is_attacked(self.board_state.kingSq[self.board_state.side], self.board_state.side ^ 1, self.board_state)
        text = self.fen_font.render(f"Check:{in_check}", True, (0, 0, 0))
        self.win.blit(text, (WIDTH, 90))
        ...
        for cap in self.captured_pieces:
            if piece_color[cap]==WHITE:
                self.win.blit(pygame.transform.scale(self.images[piece_dictss[cap]], (20, 20)),
                              pygame.Rect(WIDTH+(white_caps*11), 150, SQUARE_SIZE, SQUARE_SIZE))
                white_caps+=1
            else:
                self.win.blit(pygame.transform.scale(self.images[piece_dictss[cap]], (20, 20)),
                              pygame.Rect(WIDTH+(black_caps*11), 120, SQUARE_SIZE, SQUARE_SIZE))
                black_caps+=1
    def reset_game(self):
        self.removed_move = []
        self.white_moves = []
    ...
    def draw_all(self):
        ...
        # self.draw_info()
        self.draw_move_caps()
    def parse_game(self,fen):
        pygame.mixer.Sound.play(self.game_start_SFX)
        self.initialized_fen = fen
        self.move_comments = None
        self.best_move_recommended = None
        self.move_cap = None
        self.we_should_display_caps = False
